...
const SignUp = () => {
    ...
    const { register, handleSubmit, watch, formState: { errors, defaultValues }, setValue } = useForm({
        defaultValues: {
            firstName: "",
            lastName: "",
            companyName: "",
    ...
    return (
        <div
            className={`w-full relative bg-gray-100 flex flex-col lg:flex-row font-syne`}
        >
            <div className={isModalOpen ? "absolute h-full backdrop--sm w-full mx-auto flex justify-center items-center" : "hidden"}>
                <Modal isOpen={isModalOpen} setIsOpen={setIsModalOpen} >
                    <div className='space-y-5'>
                        <h1 className='text-center text-2xl font-syne text-softBlue'>
            ...
            <div className='h-full lg:w-[60%] py-20 flex flex-col items-center justify-center space-y-5'>
                <div className='space-y-3 text-center'>
                    <h1 className='text-2xl text-softBlue font-extrabold'>Sign Up for an Account</h1>
                ...
                <form
                    ...
                    <div className='flex text-sm lg:text-base justify-between items-center gap-3 cursor-pointer'>
                        ...
                        <select
                            ...
                            className={`
                                ...
                                ${errors && errors["role"] ? "focus:border-red-500 border-2" : ""}
                            `}
                            onChange={(e) => setUserRole(parseInt(e.target.value))}
                            defaultValue={roles.find(r => r.role === defaultValues.role)?.roleName}
                        >
                    ...
                    <div className='flex text-sm lg:text-base items-center gap-3 cursor-pointer'>
                        ...
                        <p className='text-secondary'>I accept HealthFROGS <span className='text-softBlue hover:underline'>Terms & Conditions</span></p>
                    </div>
                    <input
                        type="submit"
                        value="Sign Up"
                        className={`w-full py-2 text-white uppercase font-syne rounded-md ${acceptTerms ? "bg-softBlue cursor-pointer" : "bg-gray-400"}`}
                    />
                </form>
