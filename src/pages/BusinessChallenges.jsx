...
const BusinessChallenges = () => {
  ...
  useEffect(() => {
    const filteredChallenges = businessChallenges.filter(
  ...
  return (
    <div>
      <TopBanner heading={"Solution Partners"} img="/About/Partner.png" />
      <Container>
        <div className="py-10 space-y-10">
          <p className="flex justify-center gap-3 text-xl text-primary">
            <LogoText /> helps customers connect with solution partners who are
            proven in the field to solve challenges
          </p>
          ...
          <div className="grid grid-cols-5 gap-5">
            {Array.from({ length: 10 }).map((x) => (
              <div className="p-5 border rounded-xl bg-gray-50 space-y-3 cursor-pointer">
                <h5 className="font-poppins">Notion</h5>
                <img
          ...
          <div className="flex justify-center">
            <button
              ...
              className="text-xl font-poppins bg-softBlue text-white px-14 py-3 rounded-md"
            >
...
const DropDownMenu = ({ options, category, setCategory }) => {
  return (
    ...
    >
      ...
      {options.map((o) => (
        <option value={o.category}>{o.label}</option>
      ))}
    </select>
  );
};
