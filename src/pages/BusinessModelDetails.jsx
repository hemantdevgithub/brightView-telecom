...
const BusinessModelDetails = () => {
  ...
  useEffect(() => {
    const filteredModelStep = businessModels.filter(
      (b) => b.model === category
    );
  ...
  return (
    <div>
      <TopBanner
        img={"/Team/team.png"}
        heading={`Business Model Structure For ${category}`}
      />
      <Container>
        <div className="py-10">
          {filteredModelSteps && (
              ...
              {filteredModelSteps.map((filteredModelSteps) => (
                <div key={filteredModelSteps.id}>
                  <div className="text-center">
                    <h1 className="text-xl lg:text-3xl font-poppins flex justify-center items-center gap-5 text-blue-700">
                  ...
                  <div className="flex flex-col gap-10 my-10">
                    {filteredModelSteps &&
                      filteredModelSteps.descriptions?.map((des) => (
                        <div key={des.id} className="space-y-5">
                          <SubHeading text={des.heading} />
                          <ParaText>{des.subHeading}</ParaText>
                        </div>
                      ))}
                  </div>
                </div>
