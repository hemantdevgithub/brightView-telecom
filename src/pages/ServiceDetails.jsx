...
const ServiceDetails = () => {
    ...
    const singleService = healthcareServices.find(service => service.id === state.serviceId);
    const filteredSubService = healthcareServices.find(service => service.id === state.serviceId).services.find(s => s.id === state.subServiceId);
    return (
        <div>
            ...
            <Container>
                <div className='py-10 flex flex-col lg:flex-row gap-5'>
                    <div className='w-full lg:w-[70%] space-y-10'>
                        <h3 className='text-center text-4xl border-b-2 border-primary py-3 text-primary font-syne'>
                            {filteredSubService.service}
                        </h3>
                        <ul className='list-disc space-y-3 text-lg font-syne'>
                            {filteredSubService.points.map(p => <li>{p}</li>)}
                        </ul>
                    </div>
                    <div className='w-full lg:w-[30%] space-y-5'>
                        ...
                        <div className='flex flex-col gap-5'>
                            {
                                caseStudies.filter(s => s.category === singleService.category).map(caseStudy => (
                                    <Link
