...
const BusinessModel = () => {
  ...
  return (
    <div>
      <TopBanner heading={"Business Model"} img="/About/Business Model.png" />
      <Container>
        <div className="py-10 font-poppins space-y-10">
          <div className="space-y-4">
            <p className="text-primary font-mono text-sm font-semibold uppercase text-center">
              HealthFROGS Business Model For :
            </p>
            <div className="flex justify-center gap-5">
              <button
                onClick={() => setModelCategory("customer")}
                className={`${
                  modelCategory === "customer"
                    ? "bg-blue-500 text-white"
                    : "bg-white text-blue-500"
                } border-2 duration-300 w-[250px] rounded-lg py-2`}
              >
                Customers
              </button>
              <button
                ...
                className={`${
                  modelCategory === "solution-partners"
                    ? "bg-blue-500 text-white"
                    : "bg-white text-blue-500"
                } border-2 duration-300 w-[250px] rounded-lg py-2`}
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {steps
              .filter((s) => s.category === modelCategory)
              .map((step, index) => (
                <div
                  className={`py-10 px-5 border rounded-md shadow-md shadow-blue-400  flex flex-col items-center gap-5 hover:scale-105 duration-300 cursor-pointer`}
                >
                  <h3 className="text-xl font-poppins font-bold text-primary text-center uppercase">
                    {step.heading}
                  </h3>
