...
const ProfileSidebar = () => {
  ...
  const handleUploadImage = async (image) => {
    ...
    try {
      ...
      switch (userRole) {
        case 9238:
          const customerPhotoResponse = await uploadCustomerProfilePhoto({
            photo: image,
            publicId: public_id && public_id,
          });
          if (
            customerPhotoResponse.success === true &&
            customerPhotoResponse.statusCode === 200
          ) {
        ...
        case 7483:
          const solutionPartnerProfessionalResponse =
            await uploadSolutionPartnerProfilePhoto({
              photo: image,
              publicId: public_id && public_id,
            });
          if (
        ...
        case 2392:
          const institutionProfileResponse =
            await uploadInstitutionsProfilePhoto({
              photo: image,
              publicId: public_id && public_id,
            });
          if (
    ...
    <div className="bg-gray-50 space-y-4 lg:w-[450px] px-5 py-5">
          ...
          {user?.userRole === 9238 && (
            <>
              ...
              <h1 className="flex items-center gap-2 text-sm">
                ...
                <span className="text-softBlue font-semibold font-poppins">
      ...
      <QuickLinks
        ...
        sidebarItems={sidebarItems.filter((item) =>
          item.role.includes(user.userRole)
        )}
      />
    </div>
  );
