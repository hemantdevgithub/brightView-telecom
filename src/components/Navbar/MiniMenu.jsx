...
import { navUrls } from "../../constants/navigations";

//Main section starts from there
const MiniMenu = ({ isOpen, setIsOpen, auth }) => {
  return (
    <div
      className={`
            flex-col
    ...
    >
      ...
      <div className="lg:hidden flex justify-center mb-3"></div>
    </div>
  );
};
        ...
        >
          ...
          {submenu && (
            <AiOutlinePlus
              ...
              className={`h-7 w-7 p-1 text-blue-500 border border-blue-300 rounded-full transition-all duration-700 ${
                isSubMenuOpen ? "rotate-[225deg]" : "rotate-0"
              }`}
            />
          )}
        </NavLink>
          ...
          {submenu.map((menu) => (
            <div className="flex flex-col gap-4 border rounded-md">
              <NavLink
                className={({ isActive }) =>
                  isActive
                    ? "text-primary bg-gray-100 text-sm p-3"
                    : "text-primary text-sm p-3"
                }
                to={menu.redirect}
              >
                {menu.label}
